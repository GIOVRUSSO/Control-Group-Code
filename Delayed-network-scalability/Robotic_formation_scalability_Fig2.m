...
for j=1:layer
    for i = 1:4*j
          if any(i== ID(:,j))
                ln2 = plot(t(d+1:end),laypx(d+1:end,i,j)-laypxd(d+1:end,i,j),'k','linewidth',1)
                hold on
          else
                ln1 = plot(t(d+1:end),laypx(d+1:end,i,j)-laypxd(d+1:end,i,j),'r','linewidth',1)
          end
    end
end
set(gca,'FontSize',24)
...
legend([ln1,ln2],'Unperturbed agents','Perturbed agents','Location','best')

figure
for j=1:layer
    for i = 1:4*j
          if any(i == ID(:,j))
                ln2 = plot(t(d+1:end),laypy(d+1:end,i,j)-laypyd(d+1:end,i,j),'k','linewidth',1)
                hold on
          else
                ln1 = plot(t(d+1:end),laypy(d+1:end,i,j)-laypyd(d+1:end,i,j),'r','linewidth',1)
          end
    end
end
set(gca,'FontSize',24)
