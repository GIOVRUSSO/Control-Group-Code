 ...
 * Date: January 2020
 * Description: Implements BLE connection protocol
 * */
...
public class BLEService extends Service {
    ...
    public static final UUID Service3_UUID = UUID.fromString("0000180d-0000-1000-8000-00805f9b34fb");
    public static final UUID ThrottleCharUUID = UUID.fromString("19b10002-e8f2-537e-4f6c-d104768a1214");
...
//                Log.d("debuggg", bluetoothDevice.getAddress() + " " + bluetoothDevice.getName());
    ...
    private final BluetoothGattCallback mGattCallback = new BluetoothGattCallback() {
        ...
        public void onServicesDiscovered(BluetoothGatt gatt, int status) {
            ...
            Log.d("Gatt", "onServicesDiscovered");
            int serCount = 1;
            int charCount = 1;

            toast("Bike Service Discovered！");
            if (status == BluetoothGatt.GATT_SUCCESS) {
                Log.d("Gatt", "Gatt success");

                Log.d("Gatt", "find operation characteristics");
                if (mBluetoothGatt != null) {
                    Log.d("Gatttttttt2", "Gatt not null");
                    List<BluetoothGattService> services = mBluetoothGatt.getServices();
                    for (BluetoothGattService s : services) { // characteristics in services
                        Log.d("Gatttttttt2", "service "+serCount+": " + s.getUuid());
                        for (BluetoothGattCharacteristic c : s.getCharacteristics()) {
                            Log.d("Gatttttttt2", "service "+serCount+", character "+charCount+": " + c.getUuid());
                            charCount++;
                        }
                        serCount++;
                    }
...
//                    System.out.println(new String(readFromBLE()));
    ...
    */
     ...
     * @return
     */
    public static void wirteToBLE(int alValue){

        if(alValue==-1){alValue=100;}else if(alValue>99){alValue=99;}else if(alValue<1){alValue=1;}
        alValue=alValue-100;
        byte[] valueOut = {(byte)alValue};
        Log.d("BLE", "Commenced writeBLE");

        BluetoothGattService mGattService2 = mBluetoothGatt
                .getService(Service3_UUID);
        Log.d("BLE", "Service Found");

        BluetoothGattCharacteristic mCharacteristic2 = mGattService2
                .getCharacteristic(ThrottleCharUUID);
        Log.d("BLE", "Characteristic Found");

        if (mCharacteristic2 == null){
            Log.w("BLE", "Couldn't find characteristic");
        }
        ...
        Log.d("BLE", "Sending command "+alValue+" & bytes "+valueOut[0]);


        mCharacteristic2.setValue(valueOut);
        mBluetoothGatt.writeCharacteristic(mCharacteristic2);

        CommandSentData commandSentData = new CommandSentData(alValue);
        dbHandler.addCommandSentRow(commandSentData);
        Log.d("BLE", "Assistance Level Sent = "+alValue);
