...
//        bindService(i, BLEServiceConnection, Context.BIND_AUTO_CREATE);
    ...
    public void onClick(View view) throws InterruptedException {
        ...
        int alValue = Integer.parseInt(enterCommand.getText().toString());
        write(alValue);
    }
    ...
    public static void write(int alValue) throws InterruptedException {
        if (alValue > 0) {
            Log.i(tag, "Sent message is are " + alValue);
            BLEService.wirteToBLE(-1);
            BLEService.wirteToBLE(alValue);
        }
    }
