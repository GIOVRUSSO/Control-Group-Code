...
sumo_add_path = "sumo_files/add.add.xml" #Actually useless, could be an idea to refactor the agent class to be able to delete this (for now the agent class needs an add path but doesn't use it)
#sumo_routes_sim_path = "sumo_files/routes.rou.xml" #Commenting this out at is doesn't seem useful
#route_target_path = "sumo_files/target/" #Idem
...
def update_parking_reward():
    #Huigh reward for non-full lots, negative reward for full ones
    r1 = [0]*len(edge_list)
    for p in parkings:
...
if __name__ == '__main__':
            ...
            if "agent." + str(i) in traci.vehicle.getIDList(): #Could be replaced by 'for id in IDList'
                ...
                if past_edge[i] != current_edge and current_edge[
                    ...
                    if current_edge in parkings and traci.parkingarea.getVehicleCount(parkings[current_edge])<50: #Park if we can
                        ...
                        new_state = crowds[i].merging_DM(state, 5, state_space(state, 5), r)
                ...
                if current_edge in parkings and traci.parkingarea.getVehicleCount(parkings[current_edge])==50 and agents[i].id_goal==1:
                    ...
                    rerouteAgent(i)
                    new_state = crowds[i].merging_DM(state, 5, state_space(state, 5), r) #Redo DM loop after rerouting
                    prox_edge = edge_dict[new_state]
                    traci.vehicle.changeTarget('agent.'+str(i), prox_edge)
    ...
    #Save results
    np.save('merge_logs_run'+str(sys.argv[1])+'.npy',np.array(logs))
    np.save('merge_foe_tR'+str(sys.argv[1])+'.npy',np.array(foe_tR))
    np.save('merge_agent_tR'+str(sys.argv[1])+'.npy',np.array(agent_tR))
