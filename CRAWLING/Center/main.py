...
#Change to road ID
badInds = [1117,1043] #Indexes corresponding to blocked roads
badIDs = ['50718369','50701871']
badIDs = ['50718369','93338846#1','93338846#2']

def getEdgeIndex(id):
    return([x for x in edge_dictcars if edge_dictcars[x] == id][0])

def setRiskReward():
    r = [0]*len(edge_listcars)
    for id in badIDs:
        i = getEdgeIndex(id)
        traci.edge.setMaxSpeed(id, 0.2) #Set low speed
        r[i] = -20 #set negative reward
    return(np.array(r))
...
number_lots = {'672732730':spots, '-672732731':spots, '-672732728':spots, '401420254#0':spots, '401420254#4':spots, 
                '-1001198066':spots, '744932253':spots, '670934106':spots, '670934108' :spots, 
                '160821659#2':spots, '671983008':spots, '92961457#3':spots, '92961457#4':spots, '92961462':spots,
                '672273418#3':spots, '673737658#3':spots, '673737658#6':spots, '-565381618#0':spots, '163563222':spots, 'E5':spots, 'E6':spots}
...
if __name__ == "__main__":
    ...
    n_agents = len(agSt)
    n_foes = len(foeSt)
    #speed_meanagents = [0]*n_agents#[ [] for _ in range(n_agents) ]
    #speed_meanfoes= [0]*n_agents#[ [] for _ in range(n_foes) ]

    #co2_meanfoes = [0]*n_agents#[ [] for _ in range(n_foes) ]
    #co_meanfoes = [0]*n_agents#[ [] for _ in range(n_foes) ]
    #fuel_meanfoes = [0]*n_agents#[ [] for _ in range(n_foes) ]
    #noise_meanfoes = [0]*n_agents#[ [] for _ in range(n_foes) ]

    #co2_meanagents = [0]*n_agents#[ [] for _ in range(n_agents) ]
    #co_meanagents = [0]*n_agents#[ [] for _ in range(n_agents) ]
    #fuel_meanagents = [0]*n_agents#[ [] for _ in range(n_agents) ]
    #noise_meanagents= [0]*n_agents#[ [] for _ in range(n_agents) ]
    ...
    sumoCmd = [sumoBinary, "-c", sumocfg_name,"--step-length","0.1"] #   The last parameter is the step size, has to be small
    traci.start(sumoCmd)
    ...
    rR = setRiskReward()
    r = rR + pR #computeReward(pR, tR, pedR) 
    ...
    while len(getFreeParking(parkings, number_lots)) != 0 and  step < termination_steps: 
        ...
        for i in range(n_agents):
            ...
            if agentID in traci.vehicle.getIDList():
                ...
                current_edge = traci.vehicle.getRoadID(agentID) #get current edge
                    ...
                    #speed_meanagents[i]=traci.vehicle.getSpeed(agentID)
                    #co2_meanagents[i]=traci.vehicle.getCO2Emission(agentID)
                    #co_meanagents[i]=traci.vehicle.getCOEmission(agentID)
                    #fuel_meanagents[i]=traci.vehicle.getFuelConsumption(agentID)
                    #noise_meanagents[i]=traci.vehicle.getNoiseEmission(agentID)
                ...
                if (past_edge[i] == '' or past_edge[i] != current_edge) and current_edge[0] != ":":  #: to avoid calculating the route to an intersection
                    state = [x for x in edge_dictcars if edge_dictcars[x] == current_edge][0] #get state index
                    print('reward ', r[state],' link ', current_edge)
                    print('Actual id ', current_edge)
                    print('Guessed id ', edge_dictcars[state])
        ...
        for i in range(n_foes): #for each foes
            ...
            if foeID in traci.vehicle.getIDList(): #if the foe is in the simulation
                    ...
                    #speed_meanfoes[i]=traci.vehicle.getSpeed(foeID)
                    #co2_meanfoes[i]=traci.vehicle.getCO2Emission(foeID)
                    #co_meanfoes[i]=traci.vehicle.getCOEmission(foeID)
                    #fuel_meanfoes[i]=traci.vehicle.getFuelConsumption(foeID)
                    #noise_meanfoes[i]=traci.vehicle.getNoiseEmission(foeID)
