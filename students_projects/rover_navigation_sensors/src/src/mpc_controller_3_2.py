...
class ControllerMPC:
    ...
    def update(self, target):
        ...
        states = numpy.array([actual_position[0]+0.5, actual_position[1]+0.5, actual_position[5]]).reshape(-1, 1)  
        ...
        print(states[1]-ct[1])
        print(ct[1])
        print('the current state is', states) #3x1
        print('target is', target)
        #print('the x is', self.model.x )
        #print('the aimed target state is', self.model.tvp)
        
        self.pose_log.append([states[0],states[1]])
        np.save('pose_log.npy',self.pose_log)
        
        self.index+=1
        #if abs(states[0]-ct[0])<=0.1 and abs(states[1]-ct[1])<=0.1: #Grid square size: 0.3m, rover size 0.7x1m   
        #if (states[0]-ct[0])**2+(states[1]-ct[1])**2<=0.4 or (u[0])**2+(u[1])**2 <= 0.6: 
        #if np.abs(u[0]) <= 0.65 and np.abs(u[1]) <= 0.6: 
        #    self.index+=1
        #    print('*******************index index index', self.index, '*******************************')

        if (states[0]-target[0])**2+(states[1]-target[1])**2<=0.2:
