...
class ControllerKLCOnline:
    ...
    def __init__(self, goal, mode):
        ...
        # pose = self.state_manager_node.get_pose()      
        # self.init_position = [pose.position.x, pose.position.y]
        print("Initial position: " + str(self.init_position))
        ...
        for j in range(self.zsim): #Perform simulations
            ...
            for i in range(nSteps): #For each step 
                ...
                for action in actions:
                    ...
                    #print(is_good_state)
                    if -1 <= is_good_state[0] < self.zdiscr[0]*self.zstep[0] and -1 <= is_good_state[1] < self.zdiscr[1]*self.zstep[1]:
                        state_index = 0
                ...
                for k in possible_states:
                    diagMinusQ[k,k] = np.exp(-self.dynamic_cost(self.stateVect[k]))
    ...
    def dynamic_cost(self, state):
        ...
        for obs in obs_points:
            if(self.is_obstacle_in_fov(state[0], state[1], obs[0], obs[1], 1.0) == True):
                obs_point  = np.array([obs[0], obs[1]])
                gauss_sum += 150*my_logpdf(state[:2],obs_point,covar)
        
        cost = 0.25*(state[0] - self.xd) ** 2 + 0.25*(state[1] - self.yd) ** 2 + gauss_sum 
