...
:return: Transform representing the rover position in the world frame.
"""
def get_position():
    T_baselink_world = get_link_states('husky2::base_link', 'world') #its the same as T_baselink_world
    TBW = t.concatenate_matrices(t.translation_matrix([T_baselink_world.link_state.pose.position.x, T_baselink_world.link_state.pose.position.y, T_baselink_world.link_state.pose.position.z]),
            t.quaternion_matrix([T_baselink_world.link_state.pose.orientation.x, T_baselink_world.link_state.pose.orientation.y, T_baselink_world.link_state.pose.orientation.z, T_baselink_world.link_state.pose.orientation.w]))
...
:param init_position: Initial position transform.
:param obs_position_world: Obstacle position in the odom frame.
:return: Transform representing the obstacle position in the odom frame.
"""
