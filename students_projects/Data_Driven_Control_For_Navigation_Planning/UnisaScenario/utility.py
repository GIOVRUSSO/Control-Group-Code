        ...
        for i in range(len(edge_list)):
            if edge_list[i] not in parkings:
                ...
                if len (traci.edge.getLastStepVehicleIDs(edge_list[i].getID())) == 0:
                    ...
                    veh_id = traci.edge.getLastStepVehicleIDs(edge_list[i].getID())[0] #to get lane_ID
                    lane_ID = traci.vehicle.getLaneID(veh_id)
                    lane_length = traci.lane.getLength(lane_ID)
                    density = (veh_number)/lane_length
                if (density*100 > 1):
                    r2[i] = density #The higher the density, the lower the reward (PS: It was found that the density values were in the order of 10^-2.)
                else: 
                    r2[i] = 100 #Lower density (or no) edges will get higher rewards
    return(np.array(r2))
...
def rerouteAgent(agentI, parkings, agents, crowds, number_lots, parking_goals, parking_coordinates):
    ...
    agentID = "agent." + str(agentI)
    x, y = traci.vehicle.getPosition(agentID)
    distances = []
    for freep in l:
        p_coord = parking_coordinates[freep]
        distances.append([freep, traci.simulation.getDistance2D(x, y, p_coord[0], p_coord[1], isGeo=False, isDriving=True)])

    distsorted = sorted(distances, key=itemgetter(1))
    #The new target behavior is chosen accordingly to the current position of the vehicle.
    idgoal = [i for i in parking_goals if parking_goals[i]== distsorted[0][0]]
    print(f"idgoal: {idgoal} - parking area: {distsorted[0][0]}")
    agents[agentI].id_goal = idgoal[0]
    crowds[agentI].nTarget = idgoal[0]
    return distsorted[0][0]
